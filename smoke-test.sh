...
commits and pull requests in jefeish/cascading-auto-merge-test. A final GitHub
issue reports the starting conditions, scenario results, and all observed PRs.
EOF
}
...
finalize() {
    ...
    if [[ "$report_active" == true && "$report_published" == false ]]; then
        ...
        publish_smoke_report "$exit_code"
    fi
...
finish_scenario() {
    ...
    SCENARIO_REPORT_FILES+=("$section_file")
...
append_starting_conditions() {
    ...
    {
        printf '\n### Repository cascade configuration\n\n'
        printf '```yaml\n'
...
publish_smoke_report() {
    local exit_code="$1"
    local overall_status overall_icon issue_title issue_url section_file
    ...
    issue_title="Cascading merge smoke test ${overall_status}: ${scenario} (${run_started_at})"
