...
branch_precedes() {
    ...
    read -r -a left_tokens <<<"$(tokenize_branch "$left")"
    read -r -a right_tokens <<<"$(tokenize_branch "$right")"

    for ((index = 0; index < ${#left_tokens[@]} || index < ${#right_tokens[@]}; index++)); do
        [[ "${left_tokens[index]-}" == "${right_tokens[index]-}" ]] && continue
        ((index < ${#left_tokens[@]})) || return 0
        ((index < ${#right_tokens[@]})) || return 1

        left_number=""
        right_number=""
        [[ "${left_tokens[index]}" =~ ^([0-9]+) ]] && left_number="${BASH_REMATCH[1]}"
        [[ "${right_tokens[index]}" =~ ^([0-9]+) ]] && right_number="${BASH_REMATCH[1]}"

        if [[ -n "$left_number" && -n "$right_number" ]]; then
            ((10#$left_number < 10#$right_number)) && return 0
            return 1
        fi
        [[ -n "$left_number" ]] && return 0
        [[ -n "$right_number" ]] && return 1
        [[ "${left_tokens[index]}" < "${right_tokens[index]}" ]] && return 0
        return 1
    done
...
derive_expected_cascade() {
    local config prefix branch candidate index insertion_index entry_index=-1 expected_path
    local -a release_branches=() ordered_branches=()

    config=$(gh api \
        -H "Accept: application/vnd.github.raw+json" \
        "repos/${EXPECTED_REPO}/contents/.github/cascading-merge.yml?ref=main")
    ref_branch=$(awk -F: '/^[[:space:]]*ref_branch:/ { value=$2; gsub(/[[:space:]'\''"]/, "", value); print value; exit }' <<<"$config")
    max_merge_depth=$(awk -F: '/^[[:space:]]*maxMergeDepth:/ { value=$2; gsub(/[[:space:]]/, "", value); print value; exit }' <<<"$config")
    [[ -n "$ref_branch" ]] || fail "Remote cascading-merge.yml must define ref_branch"
    [[ "$max_merge_depth" =~ ^[1-9][0-9]*$ ]] || fail "Remote cascading-merge.yml must define a positive maxMergeDepth"

    prefix=${ENTRY_RELEASE_BRANCH%%[0-9]*}
    while IFS= read -r branch; do
        [[ "$branch" == "$prefix"* ]] && release_branches+=("$branch")
    done < <(git ls-remote --heads origin | awk '{ sub("refs/heads/", "", $2); print $2 }')

    for candidate in "${release_branches[@]}"; do
        insertion_index=${#ordered_branches[@]}
        for ((index = 0; index < ${#ordered_branches[@]}; index++)); do
            if branch_precedes "$candidate" "${ordered_branches[index]}"; then
                insertion_index=$index
                break
            fi
        done
        ordered_branches=(
            "${ordered_branches[@]:0:insertion_index}"
            "$candidate"
            "${ordered_branches[@]:insertion_index}"
        )
    done

    for ((index = 0; index < ${#ordered_branches[@]}; index++)); do
        if [[ "${ordered_branches[index]}" == "$ENTRY_RELEASE_BRANCH" ]]; then
            entry_index=$index
            break
        fi
    done
    ((entry_index >= 0)) || fail "Missing remote entry branch: $ENTRY_RELEASE_BRANCH"
    ((entry_index + max_merge_depth < ${#ordered_branches[@]})) ||
        fail "Not enough release branches after ${ENTRY_RELEASE_BRANCH} to exercise maxMergeDepth=${max_merge_depth}"

    expected_sources=()
    expected_targets=()
    for ((index = 0; index < max_merge_depth; index++)); do
        expected_sources+=("${ordered_branches[entry_index + index]}")
        expected_targets+=("${ordered_branches[entry_index + index + 1]}")
    done
    natural_target_after_limit="${ordered_branches[entry_index + max_merge_depth + 1]-}"
    expected_sources+=("${ordered_branches[entry_index + max_merge_depth]}")
    expected_targets+=("$ref_branch")

    expected_path=${expected_sources[0]}
    for ((index = 0; index < ${#expected_targets[@]}; index++)); do
        expected_path+=" -> ${expected_targets[index]}"
    done
    log "Expected cascade: ${expected_path}"
}
...
preflight() {
    ...
    for branch in "$ENTRY_RELEASE_BRANCH" "$ref_branch"; do
...
run_standard() {
    ...
    if [[ -n "$natural_target_after_limit" ]]; then
        cascade_pr=$(find_cascade_pr "${expected_sources[max_merge_depth]}" "$natural_target_after_limit" "$origin_pr")
        [[ -z "$cascade_pr" ]] || fail "Cascade exceeded maxMergeDepth=${max_merge_depth} with PR #${cascade_pr}"
    fi
    assert_branch_value "$ref_branch" "$value"
    delete_patch_branch
...
run_conflict() {
    ...
    assert_branch_value "$ref_branch" "$resolved_value"
    delete_patch_branch
