...
current_scenario_purpose=""
current_scenario_expectation=""
current_pr_file=""
current_origin_pr=""
current_stalled_pr=""
...
begin_scenario() {
    ...
    current_scenario_purpose="$3"
    current_scenario_expectation="$4"
...
finish_scenario() {
...

    {
        printf '### %s\n\n' "$current_scenario_title"
        printf '%s\n\n' "$current_scenario_purpose"
        printf 'Expected app behavior: %s\n\n' "$current_scenario_expectation"
        printf '* Status: %s **%s**\n' "$status_icon" "$status"
        printf '* Started: `%s`\n' "$current_scenario_started_at"
        printf '* Result: %s\n\n' "$summary"
...
run_standard() {
    ...
    begin_scenario \
        standard \
        "Standard cascade" \
        "Verifies that a normal merge starts a cascade through the configured branch sequence." \
        "Merge each eligible hop, respect maxMergeDepth, then perform the final merge to ${REF_BRANCH}."
...
run_conflict() {
    ...
    begin_scenario \
        conflict \
        "Merge conflict and protected-branch repair" \
        "Verifies that a cascade can recover from a merge conflict when the protected source branch cannot be updated directly." \
        "Create a repair PR, retry the stalled cascade PR after the repair merges, and continue with the original remaining depth."
...
run_existing_pr() {
    ...
    begin_scenario \
        ...
        "Verifies that the cascade pauses when an open PR already exists for the required branch pair." \
        "Attach continuation metadata to the existing PR and resume from it after it merges without resetting the remaining depth."
