...
Every invocation creates one final GitHub issue that summarizes the complete
smoke-test run. The issue includes the starting conditions, expected branch
sequence, scenario results, and links to every originating, cascade, collision,
and conflict-repair pull request observed by the harness.
...
9. After the requested scenario or scenarios finish, the exit trap creates one
   final smoke-test report issue. It also attempts to publish partial results
...
## Final smoke-test report issue

The harness creates one final issue for `standard`, `conflict`, `existing-pr`,
or `all`. The issue title includes the requested scenario, overall result, and
UTC start timestamp.
...
- A hop table distinguishing depth-counted hops from the forced final
  `ref_branch` merge
- The repository cascade configuration read from `main`
...
The issue is created from the exit trap, so a failed assertion still produces a
partial report whenever GitHub remains reachable. Failure to create the report
does not replace the original smoke-test exit status.
