...
The harness uses this short cascade path:
...
```bash
./smoke-test.sh all --preflight-only
```
...
- `standard` merges a patch PR into the source release branch, verifies both
   cascade PRs merge, and confirms the fixture reaches `development`
