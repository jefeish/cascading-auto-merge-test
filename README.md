...
the release branches from the target repository and applies the App's
branch-ordering rules with the harness's configured `ref_branch` and
`maxMergeDepth`. With the current repository state, the expected path is:
...
- `conflict` creates competing edits on adjacent release branches, verifies the
  cascade stops, resolves the real Git conflict through the App-created repair
  PR, and confirms the App merges the stalled PR and resumes through
  `development`
- `existing-pr` opens the release-to-release PR before triggering the cascade,
  then verifies the App reports the existing-PR collision and resumes after
  that PR is merged

Each scenario creates its own final GitHub issue containing the starting
configuration, expected branch sequence, result, and links to the originating,
cascade, collision, and repair PRs. `all` therefore creates three report issues.
Failed scenarios publish the partial results collected before the failure
whenever GitHub remains reachable.
...
Scenario work happens in a temporary clone. Temporary patch branches are
deleted after successful assertions.
...
source-20260925125521

resolved-source-20260925125521
