...
SMOKE_TEST_GUIDE="SMOKE-TESTS.md"

verify_ref_contains_smoke_test_guide() {
    local ref="$1"

    if ! git cat-file -e "${ref}:${SMOKE_TEST_GUIDE}" 2>/dev/null; then
        echo "ERROR: ${SMOKE_TEST_GUIDE} is missing from ${ref}; refusing to reset branches." >&2
        return 1
    fi
}
...
is_desired_branch() {
    ...
    for desired_branch in "${desired_branches[@]}"; do
        if [[ "$candidate" == "$desired_branch" ]]; then
            return 0
        fi
    done
...
verify_ref_contains_smoke_test_guide main
...
for branch in $all_local_branches; do
    if ! is_desired_branch "$branch"; then
...
verify_ref_contains_smoke_test_guide HEAD
